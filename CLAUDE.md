...
**C5 + C6 (AHORA + Cierre) — core built, two real gaps left open (see below). Next component is C1.**
...
**Two real gaps left open, found while building C5 — surface before considering Fase 1 done:**
- **No default `User` record is ever seeded.** `ConfigRepository.getUser()` returns `null` until something calls `saveUser()` once — nothing in the app does that yet, so `NowScreen` would show "no hay tarea disponible" forever on a fresh real device until a `User` is created somehow. No screen for creating/editing `User`/`FixedEvent` exists either (not in any Fase 1 doc — the docs never describe a settings UI) — needs the user's decision on how she wants to seed/manage this data for real use, not just tests.
- **`User.protectedPersonalMinutes` (the 45-min block from §34, CP-11) is not enforced anywhere.** `currentWindow()` only protects sleep (via `sleepTarget` as a fallback boundary) — CP-11 requires *both* sleep and the 45-minute personal-time block to be hard constraints. Personal time isn't wired in at all yet.
